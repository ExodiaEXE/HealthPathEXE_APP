...
class _CompanionSceneState extends State<CompanionScene>
  ...
  Widget build(BuildContext context) {
    ...
    final roomImage = CompanionAssets.roomImageAssets[theme];
          ...
          if (roomImage != null)
            Positioned.fill(
              child: Image.asset(
                roomImage,
                fit: BoxFit.cover,
                errorBuilder: (context, error, stackTrace) => _roomBackdropFallback(),
              ),
            )
          else
            Positioned.fill(child: _roomBackdropFallback()),
          // Semi-transparent overlay so mascot is visible
          Positioned.fill(
            child: Container(
              decoration: BoxDecoration(
  ...
  /// Fallback gradient khi không có ảnh phòng.
  Widget _roomBackdropFallback() {
    return Container(
      decoration: const BoxDecoration(
        gradient: LinearGradient(
          begin: Alignment.topCenter,
          end: Alignment.bottomCenter,
          colors: [Color(0xFFF8FAF8), Color(0xFFF0F4F0)],
        ),
      ),
    );
  }
